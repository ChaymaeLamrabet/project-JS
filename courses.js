...
function ajouterFiltre(categ){
 ...
 var t=[];
 var verif;
 //boucler sur chaque element de cours
 for(var q=0;q<cours.length;q++){
   //boucler sur chq e de t
   for(var i=-1;i<t.length;i++){
     //testere si l elmt cours est egale a un e de t
     if(cours[q].category.toUpperCase() ==t[i]){
       //si oui on quite la boulcle
       verif=true;
       break;
     }
     else{
   ...
   //si non on ajoute la valeur en majuscule
   if(verif==false){
     t.push(cours[q].category.toUpperCase());
   }
 }
 //ppour chaque element dans t en ajoute un element html
 t.forEach(e=>
 ajouterFiltre(e)
 )
...
function filtrerPar(c){
    //vider la balise
    content.innerHTML="";
    //si on veut consulter toutes les categs
    console.log(c);
    if(c=='ALL'){
        cours.forEach((v)=>{
